...
layout(binding = 1, rgba32f) readonly  uniform image2D texture0;

uniform float saturation = 1.2;
uniform int mode = 0; // aces as default
...
void main() {
    ...
    vec3 color = imageLoad(texture0, pixelCoords).rgb;
    ...
    imageStore(img_output, pixelCoords, vec4(mapped, 1));
}